...
az containerapp job create \
    ...
    --trigger-type Event \
    --max-executions 10 \
    --env-vars "AZURE_STORAGE_ACCOUNT_URL=$STORAGE_ACCOUNT_NAME.queue.core.windows.net" "AZURE_STORAGE_QUEUE_NAME=$QUEUE_NAME" "AZURE_STORAGE_DEADLETTER_QUEUE_NAME=$DEADLETTER_QUEUE_NAME" "CLOUD_ENVIRONMENT=Azure" "AZURE_CLIENT_ID=$MANAGED_IDENTITY_CLIENT_ID" "AZURE_STORAGE_ACCOUNT_KB_NAME=$STORAGE_ACCOUNT_NAME" "AZURE_STORAGE_CONTAINER_KB_NAME=$INGEST_CONTAINER_NAME" "COSMOS_DB_ACCOUNT_URL=$COSMOS_DB_ACCOUNT_URL" "COSMOS_DB_ACCOUNT_NAME=$COSMOS_DB_ACCOUNT_NAME" "COSMOS_DB_DATABASE_NAME=$COSMOS_DB_DATABASE_NAME" "COSMOS_DB_CONTAINER_NAME=$COSMOS_DB_CONTAINER_NAME" \
